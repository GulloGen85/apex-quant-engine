...
# --- CSS MOBILE AD ALTO CONTRASTO & TOUCH FRIENDLY ---
st.markdown("""
<style>
...
# --- CANALE NOTIFICHE PUSH (NTFY) & TELEGRAM ---
NTFY_TOPIC = "apex_signals_gullo"  # Puoi personalizzare questo nome canale
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")

def send_push_notification(title: str, message: str, priority: str = "high"):
    # 1. Push Notifica Nativa Android (Ntfy)
    try:
        requests.post(
...
def fetch_mobile_matrix():
    ...
    for item in ASSETS:
        ...
        if item["binance"]:
            try:
                p_res = requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={item['binance']}", timeout=2).json()
